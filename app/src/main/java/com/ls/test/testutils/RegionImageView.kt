...
class RegionImageView : View {
    ...
    private var scaleFactorOrigin = 1f //use to remember the last scale factor
    ...
    init {
        ...
        scaleGestureDetector = ScaleGestureDetector(context, object : ScaleGestureDetector.SimpleOnScaleGestureListener() {
            ...
            override fun onScaleBegin(detector: ScaleGestureDetector): Boolean {
                ...
                if (!rawPosition.isInBitmap()) {
                    Log.d("RegionImageView"," $rawPosition is not in bitmap")
                    focusPoint.set(1f, 1f)
                    return false
                }
                focusPoint.set(detector.focusX, detector.focusY)
                scaleFactorOld = this@RegionImageView.matrixDraw.getScale()
                Log.d("RegionImageView","onDraw : onScaleBegin .....--------------------")
                return true
            }
        ...
        gestureDetector = GestureDetector(context, object : GestureDetector.SimpleOnGestureListener() {
            override fun onScroll(e1: MotionEvent, e2: MotionEvent, distanceX: Float, distanceY: Float): Boolean {
                Log.d("RegionImageView","onScroll $distanceX,$distanceY")
                moveImageBy(-distanceX, -distanceY)
                return true
            }

            override fun onDoubleTap(e: MotionEvent): Boolean {
                Log.d("RegionImageView","aaaa 1111111111111111111111")
                focusPoint.x = e.x
                focusPoint.y = e.y
                if(matrixDraw.getScale() >= 2){
                    scaleImage(scaleFactorOrigin, focusPoint)
                }
                else{
                    scaleImage(2F, focusPoint)
                }
    ...
    private inner class AutoScale(focus: PointF,scale: Float): Runnable{

        private val mFocus = focus
        private val mTargetScale = scale
    ...
    fun displayHDImage() {
        ...
        Log.d("RegionImageView","onScaleEnd : $displayRegionRaw scale : ${optionRegion.inSampleSize}")
    ...
    fun moveImageBy(dx: Float, dy: Float) {
        Log.d("RegionImageView","moveImageBy : $dx ,$dy")
    ...
    fun scaleImage(scale: Float, focus: PointF) {
        ...
        matrixDraw.apply {
            ...
            Log.d("RegionImageView","scaleImage -- focus: $focus , scale: $scale oldScale = $oldScale")
    ...
    override fun onDraw(canvas: Canvas) = bitmap?.let {
        ...
        Log.d("RegionImageView","options scale : ${options.inSampleSize}")
        Log.d("RegionImageView","matrix scale : ${matrixDraw.getScale()}")
...
//        })
    ...
    private fun getSuspensionRect(): RectF {
        ...
        if (bitmapWidth > bitmapHeight) {
            return rectFPool.get().apply {
                set(width - Util.dp2px(SIZE_SUSPENSION.toFloat()).toFloat(), 0f
                        , width.toFloat(), (Util.dp2px(SIZE_SUSPENSION.toFloat()).toFloat() / bitmapWidth) * bitmapHeight)
            }
        } else {
            return rectFPool.get().apply {
                //adapt the image
                set(width - (Util.dp2px(SIZE_SUSPENSION.toFloat()) / bitmapHeight) * bitmapWidth, 0f,
                        width.toFloat(), Util.dp2px(SIZE_SUSPENSION.toFloat()).toFloat())
            }
        }
    }
    ...
    private fun loadImage(){
        with(options) {
            ...
            val scale = if(outWidth < outHeight) outHeight.toFloat() / measuredHeight else outWidth.toFloat() / measuredWidth
            inSampleSize = getUsableInSampleSize(scale)

            inJustDecodeBounds = false
            bitmap = BitmapFactory.decodeFile(imagePath, this)
            scaleFactorOrigin = measuredWidth / bitmapWidth.toFloat()
            matrixDraw.reset()
            Log.d("RegionImageView","onGlobalLayout : scaleFactor = $scaleFactorOrigin")
            matrixDraw.postScale(scaleFactorOrigin, scaleFactorOrigin)
            //move picture to center
            matrixDraw.postTranslate(0f, measuredHeight / 2f - (bitmapHeight * scaleFactorOrigin) / 2)
        }
        invalidate()
    }
