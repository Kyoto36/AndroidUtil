...
public class FrameAnimationUtil {
        ...
        loadRaw(resourceId, imageView.getContext(),
                new OnDrawableLoadedListener() {
                    ...
                    public void onDrawableLoaded(List<Frame> frames) {
                        ...
                        animateRawManually(frames, imageView, onComplete);
                    }
                });
    }
                            ...
                            for (int i = 0; i < parser.getAttributeCount(); i++) {
                                if (parser.getAttributeName(i).equals(
                                    ...
                                    int resId = Integer.parseInt(parser
                                            .getAttributeValue(i)
                                            .substring(1));
                                    bytes = FileUtils.inputStream2Bytes(context
                                            .getResources()
    ...
    // 4
    private static void animateRawManually(List<Frame> frames,
                                           ImageView imageView, Runnable onComplete) {
        animateRawManually(frames, imageView, onComplete, 0);
    }

    // 5
    private static void animateRawManually(final List<Frame> frames,
                ...
                if (parser.getName().equals("item")) {
                    ...
                    for (int i = 0; i < parser.getAttributeCount(); i++) {
                        if (parser.getAttributeName(i).equals("drawable")) {
                            ...
                            byte[] bytes = FileUtils.inputStream2Bytes(imageView
                                    .getContext().getResources()
