...
public class PickerView extends View {
    ...
    private int mLightColor, mDarkColor,mLineColor;
    private float mHalfWidth, mHalfHeight, mQuarterHeight, mFifthHeight;
    // 这个是liuwan1992写的，咱暂时改不动，就先放着了
    private float mMinTextSize, mTextSizeRange;
    private float mTextSpacing, mHalfTextSpacing;

    // 这个是新增的字体大小，可以xml设置
    private float mTextSizeMin,mTextSizeMax;
    ...
    public PickerView(Context context, AttributeSet attrs) {
        ...
        mLightColor = a.getColor(R.styleable.PickerView_lightColor, Color.parseColor("#59B29C"));
        mDarkColor = a.getColor(R.styleable.PickerView_darkColor,Color.parseColor("#333333"));
        mLineColor = a.getColor(R.styleable.PickerView_lineColor,Color.parseColor("#333333"));
        mTextSizeMin = a.getDimension(R.styleable.PickerView_minTextSize,Util.Companion.dp2px(12F));
        mTextSizeMax = a.getDimension(R.styleable.PickerView_maxTextSize,Util.Companion.dp2px(15F));
        a.recycle();
        initPaint();
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        // 二分之一的高度减去文字高度和行间距的高度的一半
        float firstLineY = mHalfHeight - (mMinTextSize + mTextSizeRange + mHalfTextSpacing / 2) / 2;
        float secondLineY = mHalfHeight + (mMinTextSize + mTextSizeRange + mHalfTextSpacing / 2) / 2;
        mPaint.setColor(mLineColor);
        canvas.drawRect(0, firstLineY - Util.Companion.dp2px(1F) / 2, getWidth(), firstLineY + Util.Companion.dp2px(1F) / 2, mPaint);
        canvas.drawRect(0, secondLineY - Util.Companion.dp2px(1F) / 2, getWidth(), secondLineY + Util.Companion.dp2px(1F) / 2, mPaint);
        // 绘制选中的 text
    ...
    private void drawText(Canvas canvas, int textColor, float offsetY, String text) {
        ...
        Log.e("drawText", "scale = " + scale + "   " + text);
        scale = scale < 0 ? 0 : scale;
        mPaint.setFakeBoldText(scale == 1);
        mPaint.setTextSize(mTextSizeMin + (mTextSizeMax - mTextSizeMin) * scale);
