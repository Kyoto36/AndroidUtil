package com.ls.custom_view_library.recyclerview.animators
...
import com.ls.custom_view_library.recyclerview.animators.holder.AnimateViewHolder
import com.ls.custom_view_library.recyclerview.internal.ViewHelper.clear
...
abstract class BaseItemAnimator : SimpleItemAnimator() {
  ...
  private fun animateChangeImpl(changeInfo: ChangeInfo) {
    ...
    if (view != null) {
      if (changeInfo.oldHolder != null) mChangeAnimations.add(changeInfo.oldHolder!!)
      ...
      oldViewAnim.alpha(0f).setListener(object : AnimatorListenerAdapter() {
        ...
        override fun onAnimationEnd(animator: Animator) {
          ...
          if (changeInfo.oldHolder != null) mChangeAnimations.remove(changeInfo.oldHolder!!)
          dispatchFinishedWhenDone()
        }
      }).start()
    }
    if (newView != null) {
      if (changeInfo.newHolder != null) mChangeAnimations.add(changeInfo.newHolder!!)
      ...
      newViewAnimation.translationX(0f).translationY(0f).setDuration(changeDuration).alpha(1f)
        .setListener(object : AnimatorListenerAdapter() {
          ...
          override fun onAnimationEnd(animator: Animator) {
            ...
            if (changeInfo.newHolder != null) mChangeAnimations.remove(changeInfo.newHolder!!)
            dispatchFinishedWhenDone()
          }
        }).start()
    }
