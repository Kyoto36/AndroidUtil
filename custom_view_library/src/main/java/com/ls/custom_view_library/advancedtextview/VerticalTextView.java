...
package com.ls.custom_view_library.advancedtextview;
...
public class VerticalTextView extends AppCompatTextView {
    ...
    public float getCharSpacingExtra(){
        return this.mCharSpacingExtra;
    }
...
//            }
        ...
        Log.d(TAG,"targetSubPara " + targetSubPara);
        // 计算文本粗略高度，包括padding
        ...
        if (textLines > subTextStr.length)
            ...
            for (int i = 0; i < targetSubPara.length(); i++) {
                ...
                // 区别标点符号 和 文字
                symbolHeight = 1.4f * getCharHeight(char_i, getTextPaint());
                normalHeight = getTextSize();
                if(symbolHeight > normalHeight) {
                    textHeight += symbolHeight + charSpacingExtra;
                }
                else{
                    textHeight += normalHeight + charSpacingExtra;
                }
...
//                }
    ...
    private void drawVerticalText(Canvas canvas, float lineSpacingExtra,
        ...
        for (int j = 0; j < textStrLength; j++) {
            ...
            */
            ...
            Log.d("aaaaaaaaaaa","char_j = " + char_j + " currentLineOffsetY " + currentLineOffsetY + " getHeight() " + getHeight() + " drawPadding[3] " + drawPadding[3]);
    ...
    public float getCharHeight(String target_char){
        return getCharHeight(target_char,getTextPaint());
    }
    ...
    public float getCharWidth(String target_char){
        return getCharWidth(target_char,getTextPaint());
    }
     ...
     * @return
     */
    public boolean isVerticalSymbol(String str) {
        String regex = ".*[︵ ︷︿︹︽﹁﹃︻︶︸﹀︺︾ˉ﹂﹄︼|]$+.*";
        Matcher m = Pattern.compile(regex).matcher(str);
        return m.matches();
    }
